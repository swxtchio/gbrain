...
- **Engine parity.** `src/core/postgres-engine.ts` and `src/core/pglite-engine.ts` move in
  ...
  `test/schema-bootstrap-coverage.test.ts`). MUST-DO when a migration adds a
  column that the static schema blob (`schema.sql`/`pglite-schema.ts`) indexes:
  also add an `ADD COLUMN IF NOT EXISTS` probe to `applyForwardReferenceBootstrap`
  in BOTH engines. The blob replays (unconditional `CREATE INDEX`) BEFORE
  migrations run, and `CREATE TABLE IF NOT EXISTS` won't add the column to an
  existing table — so omitting the bootstrap wedges every long-lived brain on
  upgrade with `column "<name>" does not exist`, freezing it at the old version.
