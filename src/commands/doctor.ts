...
export async function runDoctor(engine: BrainEngine | null, args: string[], dbSource?: DbUrlSource) {
  ...
  // 1. Resolver health
  // autoDetectSkillsDir honors $OPENCLAW_WORKSPACE, ~/.openclaw/workspace,
  // and the gbrain-repo walk in that order — so `gbrain doctor` works the
  // same from any cwd as long as the env var is set, matching the priority
  // used by check-resolvable / routing-eval / skillify / skillpack.
  const skillsDirDetected = autoDetectSkillsDir();
  const skillsDir = skillsDirDetected.dir;
  if (skillsDir) {
