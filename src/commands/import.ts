...
export async function runImport(engine: BrainEngine, args: string[], opts: { commit?: string; sourceId?: string } = {}): Promise<RunImportResult> {
  ...
  async function processFile(eng: BrainEngine, filePath: string) {
    ...
    try {
      // SWX: opts.sourceId flows from sync's --source flag (or attached
      // source dotfile) through performFullSync → runImport → here, so
      // every page imported in this run lands in the active source.
